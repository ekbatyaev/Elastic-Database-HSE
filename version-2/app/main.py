...
async def get_embedding(text):
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            data = response.json()
            print("Embedding dimension:", data["dim"])
            print("First 5 values:", data["embedding"][:5])
...
async def index_documents_from_directory(es: Elasticsearch):
    try:
...
async def initialize_app():
    try:
        ...
        es = get_es_client(API_TOKEN)
        create_index(es)
        # Индексация документов
        await index_documents_from_directory(es)
...
@app.post("/search")
async def search_documents(request: SearchRequest, token: str = Header(...)):
    try:
